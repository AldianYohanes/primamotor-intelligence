...
export async function POST(
  ...
  for (const item of items ?? []) {
    if (!item.matched_product_id || !item.suggested_quantity) {
      failed.push({
        item_id: item.id,
        error: "Produk atau kuantitas belum lengkap",
      });
      continue;
    }
    ...
    if (locationError) {
      ...
      failed.push({
        item_id: item.id,
        error: "Gagal mencari lokasi toko default",
      });
      continue;
    }
    if (!defaultLocation) {
      failed.push({
        item_id: item.id,
        error: "Lokasi toko default tidak ditemukan",
      });
      continue;
    }
    ...
    if (rpcError) {
      ...
      // commit-nya).
      logger.error(
        "RPC record_stock_transaction gagal saat commit receipt import",
        {
          route: "admin/receipt-imports/commit",
          business_id: importRow.business_id,
    ...
    if (updateItemError) {
      ...
      logger.error(
        "Gagal update resulting_transaction_id di receipt_import_items",
        {
          route: "admin/receipt-imports/commit",
          business_id: importRow.business_id,
          import_id: id,
